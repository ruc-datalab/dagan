...
class Dataset:
	...
	def split(cls,
		...
		dataset_args = {'fields': fields, 'ignore_columns': ignore_columns, 'format': format, 'col_type': col_type}
	...
	def convert(self, data):
		...
		for i, col in enumerate(self.columns):
			result.append(self.__dict__[col].convert(data[:,i].reshape(-1,1)))
	...
	def reverse(self, sample):
		...
		sample_table = np.concatenate(sample_columns, axis=1)
		return sample_table
...
class Column:
	...
	def convert(self, data):
		if self.Field is None:
			...
			return self.Field.convert(data)
